def insert_into_all(item, nested_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Better to be imutable here.
    new_lst = []
    for lst in nested_list:
        new_lst.append([item]+lst)
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        with_out_first = subseqs(s[1:]) 
        return with_out_first + insert_into_all(s[0], with_out_first)
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    def helper_rest_pointer(head, iterable):
        yield head
        yield from iterable
    ...
    if next_a == None and next_b == None:
        ...
        if next_a < next_b:
            yield next_a
            yield from merge(iter_a, helper_rest_pointer(next_b, iter_b))
        elif next_a > next_b:
            yield next_b
            yield from merge(helper_rest_pointer(next_a, iter_a), iter_b)
        elif next_a == next_b:
            yield next_a
            yield from merge(iter_a, iter_b)
        # Comments: one should not assume incr_a, incr_b are not iterators,
        # which means they may have been changed when excuting next_a, nex_b = ... 
...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        return f'Accountholder: {self.holder}, Deposits: {self.deposit_times}, Withdraws: {self.withdraw_times}'
...
def trade(first, second):
    ...
    while (m < len(first) + 1) and (n < len(second) + 1) and not(sum(first[:m]) == sum(second[:n])):
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        for subtree in t.branches:
            for sub_path in long_paths(subtree, n-1):
                paths.append([t.label] + sub_path)
